...
# -----------------------------------
    ...
    def test_send_positive_gossip(self):
        ...
        helper.list_validator(network.nodes, ['dag.blocks_by_number.length'], 25)

        helper.perform_block_steps(5)
        helper.list_validator(network.nodes, ['dag.blocks_by_number.length'], 30)
    ...
    def test_negative_gossip_by_zeta(self):
        ...
        helper.list_validator(network.nodes, ['dag.blocks_by_number.length'], 2)
        ...
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 0) # not permited for gossip send

        node1.step()  # broadcast negative gossip
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 1)
        ...
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 5)
        ...
        helper.list_validator(network.nodes, ['dag.blocks_by_number.length'], 3)
        # gossips cleaned from mem pool by block handling
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 0)
        ...
        # after node2 step
        helper.list_validator(network.nodes, ['dag.blocks_by_number.length'], 4)
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 0)
    ...
    def test_maliciously_send_negative_gossip(self):
        ...
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 6)
    ...
    def test_maliciously_send_negative_and_positive_gossip(self):
        ...
        helper.list_validator(network.nodes, ['dag.transactions_by_hash.length'], 1)
        ...
        helper.list_validator(network.nodes, ['mempool.gossips.length'], 7)
        helper.list_validator(network.nodes, ['dag.transactions_by_hash.length'], 1)
        ...
        helper.list_validator(network.nodes, ['dag.transactions_by_hash.length'], tx_total_count)
        ...
        helper.list_validator(network.nodes, ['permissions.epoch_validators.length'], GENESIS_VALIDATORS_COUNT)
        ...
        for i in range(5, ROUND_DURATION * 6 + 1):
            ...
            if i == ROUND_DURATION * 6 + 1:
                ...
                helper.list_validator(network.nodes, ['permissions.epoch_validators.length'],
                                                     GENESIS_VALIDATORS_COUNT-1)
                # TODO nodes recalculates 2 times ?
                helper.list_validator(network.nodes, ['permissions.epoch_validators.epoch0.length'],
                                                    GENESIS_VALIDATORS_COUNT-1)
                                                    # maybe 20 (on default block time and round duration)
                helper.list_validator(network.nodes, ['permissions.epoch_validators.epoch1.length'],
                                                    GENESIS_VALIDATORS_COUNT-1)
                # !
#            else:
#                self.list_validator(network.nodes, ['dag.blocks_by_number.length'], i)
