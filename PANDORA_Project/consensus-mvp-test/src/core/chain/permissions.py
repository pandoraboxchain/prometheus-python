...
from core.chain.validators import Validator, Validators
...
from core.chain.stake_manager import StakeManager
from core.crypto.keys import Keys
from core.crypto.entropy import Source
from core.chain.params import SECRET_SHARE_PARTICIPANTS_COUNT
...
class Permissions:
    def __init__(self, epoch, validators=Validators()):
        ...
        if not initial_validators:
            initial_validators = Validators.get_from_file(Validators())
        self.epoch = epoch
        ...
        if not initial_signers_indexes:
            initial_signers_indexes = self.epoch.calculate_validators_indexes(genesis_hash, validator_count,
                                                                              Source.SIGNERS)

        initial_randomizers_indexes = validators.randomizers_order
        if not initial_randomizers_indexes:
            initial_randomizers_indexes = self.epoch.calculate_validators_indexes(genesis_hash, validator_count,
                                                                                  Source.RANDOMIZERS)

        self.log("Initial signers:", initial_signers_indexes[0:3], initial_signers_indexes[3:6],
                 initial_signers_indexes[6:9], initial_signers_indexes[9:12], initial_signers_indexes[12:15],
                 initial_signers_indexes[15:19])
    ...
    def get_signers_indexes(self, epoch_hash):
        if not epoch_hash in self.signers_indexes:
            ...
            self.log("Calculated signers:", random_indexes[0:3], random_indexes[3:6], random_indexes[6:9],
                     random_indexes[9:12], random_indexes[12:15], random_indexes[15:19])
